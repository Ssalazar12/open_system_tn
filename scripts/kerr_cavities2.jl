...
F_list = [0.1,0.5,1.0, 1.5, 2.0, 2.5, 3.0, 5.0]
...
for F in F_list
    ...
    # for cluster use 
    str_file_name = @sprintf("/home/user/santiago.salazar-jaramillo/open_system_tn/data/sims/L=10/%s_N%i_L%i_om%.2f_J%.2f_gamma%.2f_kerr%.2f_drive%.2f_maxt%.2f_deltat%.2f_traj%i_cutexp%i.h5",
                            evol_type,N,L,ω,J,γ,U,F,maxt,Delta_t, N_trajectories, cutoff_exponent)
